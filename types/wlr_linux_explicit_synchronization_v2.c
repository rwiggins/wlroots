...
struct wlr_linux_surface_sync_v2_commit {
	struct wlr_linux_surface_sync_v2_state state;
...
static void surface_sync_destroy(
	...
	wl_list_remove(&surface_sync->surface_client_commit.link);
...
static void surface_sync_commit_handle_fence_available(void *data) {
	struct wlr_linux_surface_sync_v2_commit *commit = data;
	commit->surface_sync->current = commit->state;
	wlr_surface_unlock_cached(commit->surface_sync->surface, commit->cached_seq);
	free(commit);
}

static void surface_sync_handle_surface_client_commit(struct wl_listener *listener,
		void *data) {
	struct wlr_linux_surface_sync_v2 *surface_sync =
		wl_container_of(listener, surface_sync, surface_client_commit);

	if (surface_sync->pending.acquire_timeline != NULL &&
			surface_sync->surface->pending.buffer == NULL) {
	...
	struct wlr_linux_surface_sync_v2_state pending = surface_sync->pending;
	surface_sync->pending = (struct wlr_linux_surface_sync_v2_state){0};

	if (pending.acquire_timeline == NULL) {
		// TODO: add support for cached state
		surface_sync->current = pending;
		return;
	}

	// Block the surface commit until the fence materializes
	// TODO: unregister on surface destroy
	struct wlr_linux_surface_sync_v2_commit *commit = calloc(1, sizeof(*commit));
	if (commit == NULL) {
		wl_resource_post_no_memory(surface_sync->resource);
		return;
	}

	commit->state = pending;
	commit->surface_sync = surface_sync;
	commit->cached_seq = wlr_surface_lock_pending(surface_sync->surface);

	// TODO: this leaks the event source
	struct wl_client *client = wl_resource_get_client(surface_sync->resource);
	struct wl_display *display = wl_client_get_display(client);
	struct wl_event_loop *loop = wl_display_get_event_loop(display);
	wlr_render_timeline_wait(pending.acquire_timeline, pending.acquire_point,
		DRM_SYNCOBJ_WAIT_FLAGS_WAIT_AVAILABLE, loop,
		surface_sync_commit_handle_fence_available, commit);
}
...
static void explicit_sync_handle_get_surface(struct wl_client *client,
	...
	struct wlr_linux_surface_sync_v2 *surface_sync = calloc(1, sizeof(*surface_sync));
	if (surface_sync == NULL) {
	...
	surface_sync->surface_client_commit.notify = surface_sync_handle_surface_client_commit;
	wl_signal_add(&surface->events.client_commit, &surface_sync->surface_client_commit);
